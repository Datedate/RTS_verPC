...
public:
	...
	bool GetKeyboardPress(int _key);
	bool GetKeyboardTrigger(int _key);
	bool GetKeyboardRelease(int _key);
	...
	long GetMouseAxisY();
	long GetMouseAxisZ();
	long GetMouseCurrentX();
	long GetMouseCurrentY();

	// Event—p
	BYTE* GetKeyState() {
		return m_keyState;
	}
	BYTE* GetKeyStateTrigger() {
		return m_keyStateTrigg;
	}
	BYTE* GetKeyStateRelease() {
		return m_keyStateRelea;
	}
	POINT GetMousePos() {
		return m_mousePos;
	}
private:
	...
	LPDIRECTINPUTDEVICE8	m_pDirDevKeyboard;
	LPDIRECTINPUTDEVICE8	m_pDirDevMouse;
	BYTE					m_keyState[NUM_KEY_MAX];
	BYTE					m_keyStateTrigg[NUM_KEY_MAX];
	BYTE					m_keyStateRelea[NUM_KEY_MAX];
	DIMOUSESTATE2			m_mouseState;
	DIMOUSESTATE2			m_mouseStateTrigg;
	DIMOUSESTATE2			m_mouseStateRelea;
	POINT					m_mousePos;
	HWND					m_hwnd;
};
