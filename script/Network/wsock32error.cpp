...
struct errdata_t errdata[] = {
	...
	, WSAENOTSOCK,
	"Th4e descriptor is not a socket."
	, WSAEOPNOTSUPP,
...
void errcom(int errcode){
	...
	int	i;
	for (i = 0; i<sizeof(errdata) / sizeof(errdata[0]); i++){
		if (errcode == errdata[i].eno){
			::MessageBox(NULL, errdata[i].errmsg, "WINSOCK ERROR !!", MB_OK);
			break;
		}
	}
}