...
*/
HostRoomMainLayer::HostRoomMainLayer()
{
...
bool HostRoomMainLayer::Init() {
	...
	matchClick->m_onMouseUp = [this](EventMouseInfo* _info) {
		...
		if (target->InRange(_info->GetPosition().x, _info->GetPosition().y)) {
			...
			if (m_recvThreadFlag) {
				...
				// マッチング
				m_recvThread = std::thread(&HostRoomMainLayer::ServerRecv,this);
	...
	SET_UPDATE(HostRoomMainLayer);

	return true;
}

void HostRoomMainLayer::Update() {
	if (!m_isnextSceneFlag) {
		SceneManager::GetInstance()->ChangeScene(SCENE::BATTLE);
	}
}

void HostRoomMainLayer::Exit() {
...
void HostRoomMainLayer::ServerRecv() {
	/*
	...
	recvData.info.header = RecvDataMode::RECV_CONNECT;
	...
	while (m_recvThreadFlag) {
		if (! m_udpSocket->Receive(recvData.data, sizeof(RecvMessageData))) {
			...
			{
			case RecvDataMode::RECV_CONNECT:
				sendData.info.header = SendDataMode::SEND_CONNECT;
				m_recvThreadFlag = false ;
				return;
			default:
				break;
